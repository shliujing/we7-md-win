...
define('CLOUD_GATEWAY_URL', 'http://we7.cloud.cmhtml.com/api/api.php');
define('CLOUD_GATEWAY_URL_NORMAL', 'http://we7.cloud.cmhtml.com/api/api.php');
...
function cloud_build() {
	...
	$dat = cloud_request('http://we7.cloud.cmhtml.com/api/api.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://we7.cloud.cmhtml.com/api/api.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://we7.cloud.cmhtml.com/api/api.php', $pars);
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://we7.cloud.cmhtml.com/api/api.php', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'http://we7.cloud.cmhtml.com/api/auth.php?__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://we7.cloud.cmhtml.com/api/api.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
